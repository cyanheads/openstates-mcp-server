...
await createApp({
  ...
  title: 'openstates-mcp-server',   // exact machine display identity
...
|:---------|:------------|
| `ctx.log` | Request-scoped logger — `.debug()`, `.info()`, `.notice()`, `.warning()`, `.error()`. Auto-correlates requestId, traceId, tenantId and also emits `notifications/message`, so treat it as client-visible. |
