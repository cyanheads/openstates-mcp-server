...
describe('billResearch', () => {
  it('generates a user message for the given bill', async () => {
    const args = billResearch.args!.parse({
      jurisdiction: 'Washington',
      session: '2025',
      bill_id: 'HB 1000',
    });
    const messages = await billResearch.generate(args);
    expect(messages).toHaveLength(1);
    expect(messages[0]!.role).toBe('user');
    expect(messages[0]!.content.type).toBe('text');
  });

  it('embeds jurisdiction, session, and bill_id in the prompt text', async () => {
    const args = billResearch.args!.parse({
  ...
  it('requests include=sponsorships,actions,votes,abstracts,versions in the prompt', async () => {
    const args = billResearch.args!.parse({
      jurisdiction: 'wa',
      session: '2025',
      bill_id: 'HB 1000',
