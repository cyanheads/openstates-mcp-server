...
describe('legislatorProfile', () => {
  it('generates a user message for the given legislator', async () => {
    const args = legislatorProfile.args!.parse({
      name: 'Jane Smith',
      jurisdiction: 'Washington',
    });
    const messages = await legislatorProfile.generate(args);
    expect(messages).toHaveLength(1);
    expect(messages[0]!.role).toBe('user');
    expect(messages[0]!.content.type).toBe('text');
  });

  it('embeds name and jurisdiction in the prompt text', async () => {
    const args = legislatorProfile.args!.parse({
      name: 'Bob Jones',
  ...
  it('instructs use of openstates tools in the prompt text', async () => {
    const args = legislatorProfile.args!.parse({
      name: 'Smith',
      jurisdiction: 'wa',
    });
  ...
  it('requests include=offices,links for contact information', async () => {
    const args = legislatorProfile.args!.parse({
      name: 'Jane Smith',
      jurisdiction: 'wa',
    });
  ...
  it('covers voting record instructions', async () => {
    const args = legislatorProfile.args!.parse({
      name: 'Jane Smith',
      jurisdiction: 'wa',
    });
